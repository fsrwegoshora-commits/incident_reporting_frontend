    ...
    try {
      ...
      if (_isRegisterMode) {
        ...
        if (registerResponse['status'] == 'Error') {
          final message = registerResponse['message'] ?? "Registration failed";
      ...
      final otpResponse = await api.requestOtp(formattedPhone);

      print("📡 OTP response: $otpResponse");

      final otpMessage = otpResponse['message'];

      if (otpResponse['status'] != 'Error') {
        print("📲 OTP sent (Message: $otpMessage)");
        ...
        final errorMessage = otpMessage ?? "Unknown error";
