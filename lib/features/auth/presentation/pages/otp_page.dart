...
class _OtpScreenState extends State<OtpScreen> with TickerProviderStateMixin {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      final response = await api.verifyOtp(widget.phoneNumber, otp);
      ...
      if (token != null && token.toString().startsWith("eyJ")) {
        ...
        final error = response['message'] ?? "Failed to verify OTP";
  ...
  Future<void> _resendOtp() async {
    ...
    try {
      ...
      final response = await api.requestOtp(widget.phoneNumber);
      ...
      if (response['status'] == 'Error') {
        final error = response['message'] ?? "Failed to resend OTP";
        _showErrorSnackBar(error);
        return;
      }

      // Check success conditions
      final success = response['status'] != 'Error';
