...
import 'package:incident_reporting_frontend/features/shift/presentation/pages/bulk_checkpoint_shift_form.dart';
import 'package:incident_reporting_frontend/features/shift/presentation/pages/officer_shift_form.dart';
...
class _OfficerShiftManagementScreenState extends State<OfficerShiftManagementScreen>
    with TickerProviderStateMixin {
        ...
        Future<Map<String, dynamic>> _fetchShifts() async {
                ...
                try {
                        final response = await _api.getShiftsByStation(
                                widget.stationUid,
                                page: _currentPage,
                                size: _pageSize,
                        );
                        ...
                        List<Map<String, dynamic>> shifts = List<Map<String, dynamic>>.from(response['data'] ?? []);
                        int totalElements = response['elements'] ?? shifts.length;
                        int totalPages = response['pages'] ?? 1;
        ...
        Future<void> _deleteShift(String uid) async {
                ...
                if (confirm == true) {
                        try {
                                final response = await _api.deleteOfficerShift(uid);
        ...
        Future<void> _submitExcuseShift(String shiftId, String reason) async {
                try {
                        final response = await _api.excuseShift(shiftId, reason);

                        if (response['status'] == 'Error') {
                                throw Exception(response['message']);
                        }

                        final message = response['message'] ?? "Excuse failed";
        ...
        Future<void> _reassignShift(String uid, String shiftDate, String startTime, String endTime) async {
                ...
                if (newOfficerUid != null) {
                        try {
                                final response = await _api.reassignShift(uid, newOfficerUid);

                                if (response['status'] == 'Error') {
                                        throw Exception(response['message']);
                                }

                                final message = response['message'] ?? "Reassign failed";
        ...
        Future<List<Map<String, dynamic>>> _fetchAvailableOfficers(String date, String startTime, String endTime) async {
                try {
                        final response = await _api.getAvailableOfficersForSlot(date, startTime, endTime);
