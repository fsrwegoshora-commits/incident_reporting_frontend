...
import 'package:incident_reporting_frontend/features/shift/presentation/pages/checkpoint_shifts_page.dart';
import 'package:incident_reporting_frontend/features/station/presentation/pages/register_traffic_checkpoint_tab.dart';
...
class _TrafficCheckpointsByStationScreenState extends State<TrafficCheckpointsByStationScreen>
  ...
  Future<Map<String, dynamic>> _fetchCheckpoints() async {
    ...
    try {
      print('Fetching checkpoints for station ${widget.stationUid}, page: $_currentPage');

      final response = await api.getCheckpointsByStation(widget.stationUid, page: _currentPage, size: _pageSize);
      ...
      final checkpoints = response['data'] ?? [];
  ...
  Future<void> _deleteCheckpoint(String uid) async {
    ...
    if (confirm == true) {
      try {
        final response = await api.deleteTrafficCheckpoint(uid);
  ...
  Future<void> _toggleCheckpointStatus(Map<String, dynamic> checkpoint) async {
    ...
    try {
      final response = await api.toggleCheckpoint(checkpointUid);
      ...
      if (isSuccess) {
        final newStatus = response['data']?['active'] ?? !currentStatus;
        ...
        _showModernSnackBar(response['message'] ?? "Failed to toggle checkpoint status", isSuccess: false);
      }
    } catch (e) {
      print("❌ Error: $e");
