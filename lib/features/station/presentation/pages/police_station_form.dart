...
class _PoliceStationFormState extends State<PoliceStationForm> {
  ...
  // Hierarchy
  String _selectedLevel = 'POLICE_STATION';
  String? _selectedParentStationUid;
  List<Map<String, dynamic>> _allStations = [];

  static const List<Map<String, String>> _stationLevels = [
    {'value': 'POLICE_POST',       'label': 'Police Post'},
    {'value': 'POLICE_STATION',    'label': 'Police Station'},
    {'value': 'DISTRICT_HQ',       'label': 'District Police HQ'},
    {'value': 'REGIONAL_HQ',       'label': 'Regional Police HQ'},
    {'value': 'ZONAL_SPECIALIZED', 'label': 'Zonal / Specialized Unit'},
    {'value': 'NATIONAL_HQ',       'label': 'National Police HQ'},
  ];
  ...
  void _initializeForm() {
    if (widget.existingPoliceStation != null) {
      ...
      _selectedLevel = widget.existingPoliceStation!['level'] ?? 'POLICE_STATION';
      _selectedParentStationUid =
          widget.existingPoliceStation!['parentStation']?['uid']?.toString();
    }
  ...
  Future<void> _loadAllStations() async {
    try {
      final response = await _api.getPoliceStations(page: 0, size: 200);
      if (mounted && response['data'] != null) {
        setState(() {
          _allStations = List<Map<String, dynamic>>.from(response['data']);
          // Exclude self when editing
          if (_uid != null) {
            _allStations.removeWhere((s) => s['uid'] == _uid);
          }
        });
      }
    } catch (_) {}
  ...
  Future<void> _loadAdministrativeAreas({String searchTerm = ''}) async {
    ...
    try {
      final response = await _api.getAdministrativeAreas(page: 0, size: 50);

      if (!mounted) return;

      if (response['data'] != null) {
        setState(() {
          _administrativeAreas = List<Map<String, dynamic>>.from(response['data']);
  ...
  Future<void> _savePoliceStation() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedLevel != 'NATIONAL_HQ' && _selectedParentStationUid == null) {
        _showModernSnackBar('Please select a parent station', isSuccess: false);
        return;
      }
      ...
      try {
        final policeStationDto = {
          ...
          if (_selectedLevel != 'NATIONAL_HQ')
            'parentStationUid': _selectedParentStationUid,
          'location': {
            'latitude': _latitude,
        ...
        final response = await _api.savePoliceStation(policeStationDto);
  ...
  Widget build(BuildContext context) {
    if (!_isInitialized) {
      return Scaffold(
        ...
        appBar: AppTopBar(
          title: widget.existingPoliceStation != null
              ? 'Edit Police Station'
            ...
            child: Column(
              ...
              children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: _stationLevels.map((level) {
                            ...
                            onChanged: (value) => setState(() {
                              ...
                              if (_selectedLevel == 'NATIONAL_HQ') {
                                _selectedParentStationUid = null;
                              }
                            }),
                            decoration: InputDecoration(
                              labelText: 'Station Level',
                              prefixIcon: const Icon(Icons.account_tree_rounded),
                              border: OutlineInputBorder(
                                  borderRadius: BorderRadius.circular(12)),
                            ),
                            validator: (v) =>
                                v == null ? 'Station level is required' : null,
                          ),
                          if (_selectedLevel != 'NATIONAL_HQ') ...[
                            const SizedBox(height: 12),
                            DropdownButtonFormField<String>(
                              value: _selectedParentStationUid,
                              items: _allStations.map((s) {
                                final level = s['level'] ?? '';
                                final levelLabel = _stationLevels
                                    .firstWhere(
                                      (l) => l['value'] == level,
                                      orElse: () => {'label': level},
                                    )['label']!;
                                return DropdownMenuItem<String>(
                                  ...
                                  child: Text('${s['name']} ($levelLabel)'),
                                );
                              }).toList(),
                              onChanged: (v) =>
                                  setState(() => _selectedParentStationUid = v),
                              decoration: InputDecoration(
                                labelText: 'Parent Station',
                                prefixIcon: const Icon(Icons.corporate_fare_rounded),
                                border: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(12)),
                              ),
                              validator: (v) => _selectedLevel != 'NATIONAL_HQ' && v == null
                                  ? 'Parent station is required'
                                  : null,
                            ),
                          ],
                          const SizedBox(height: 24),
                          ...
                          const SizedBox(height: 24),
                          AppButton(
                            label: widget.existingPoliceStation != null
                                ? 'Update Station'
                                : 'Register Station',
