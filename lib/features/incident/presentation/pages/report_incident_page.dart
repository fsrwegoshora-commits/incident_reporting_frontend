...
class _ReportIncidentScreenState extends State<ReportIncidentScreen> {
  ...
  Future<void> _loadCurrentOfficer() async {
    ...
    try {
      ...
      final response = await api.getCurrentOfficerOnDuty(widget.selectedStation['uid']);
  ...
  Future<void> _submitIncident() async {
    ...
    try {
      ...
      final response = await api.createIncident(incidentDto);

      print('📋 Response: $response');

      if (response['status'] == 'Error') {
        _showSnackBar(response['message'] ?? 'Failed to submit report', isError: true);
        return;
      }
      ...
      if (response['status'] == 'Success') {
        ...
        _showSnackBar(response['message'] ?? 'Failed to submit', isError: true);
      }
    } catch (e) {
