...
import 'package:incident_reporting_frontend/features/incident/presentation/pages/video_player_page.dart';
...
import 'package:incident_reporting_frontend/shared/services/media_download_service.dart';
import 'package:incident_reporting_frontend/shared/services/media_service.dart';
import 'package:incident_reporting_frontend/core/utils/media_url_converter.dart';
import 'package:incident_reporting_frontend/shared/services/voice_recorder_service.dart';
...
class _IncidentChatScreenState extends State<IncidentChatScreen> {
  ...
  Future<void> _loadMessages({bool silent = false}) async {
    ...
    try {
      final response = await _apiService.getAllIncidentMessages(widget.incidentUid);

      if (response['status'] == 'Error') {
        if (!silent) {
          _showSnackBar(response['message'] ?? 'Failed to load messages', isError: true);
        }
        return;
      }

      var messages = (response['data'] as List<dynamic>? ?? []).cast<Map<String, dynamic>>();
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (response['status'] == 'Error') {
        _showSnackBar(response['message'] ?? 'Failed to send message', isError: true);
        return;
      }
      ...
      if (response['status'] == 'Success') {
        ...
        _showSnackBar(response['message'] ?? 'Failed to send', isError: true);
      }
    } catch (e) {
      _showSnackBar('Error: ${e.toString()}', isError: true);
    } finally {
  ...
  Future<void> _sendMediaMessageWithDetails({
    ...
    try {
      ...
      final response = await _apiService.sendChatMessage(chatMessageDto);
      ...
      print('🔍 Send Message Status: ${response['status']}');

      if (response['status'] == 'Success') {
        _showSnackBar('${messageType.toLowerCase()} sent successfully');
        await _loadMessages(silent: true);
      } else {
        print('❌ Send failed: ${response['message']}');
        _showSnackBar(response['message'] ?? 'Failed to send media', isError: true);
      }
    } catch (e) {
      print('❌ Send error: $e');
