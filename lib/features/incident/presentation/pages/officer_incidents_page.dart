...
import 'package:incident_reporting_frontend/features/incident/presentation/pages/incident_chat_page.dart';
import 'package:incident_reporting_frontend/features/incident/presentation/pages/incident_details_page.dart';
...
class _OfficerIncidentsScreenState extends State<OfficerIncidentsScreen>
  ...
  Future<void> _loadAllIncidents({String? searchKey}) async {
    ...
    try {
      ...
      final response = isStationAdmin
          ? await api.getStationIncidents(page: 0, size: 50, status: _statusFilter)
          : await api.getOfficerIncidents(page: 0, size: 50, status: _statusFilter);
      ...
      final incidents = response['data'] as List<dynamic>? ?? [];
      setState(() {
  ...
  Future<void> _loadMyIncidents({String? searchKey}) async {
    ...
    try {
      ...
      final response = (_userRole == 'STATION_ADMIN' || _userRole == 'ROOT')
          ? await api.getStationIncidents(page: 0, size: 50, status: 'IN_PROGRESS')
          : await api.getOfficerIncidents(page: 0, size: 50, status: 'IN_PROGRESS');
  ...
  Future<void> _loadStationIncidents({String? searchKey}) async {
    ...
    try {
      final api = ApiService();
      final response = await api.getStationIncidents(page: 0, size: 50, status: _statusFilter);
  ...
  Future<void> _updateIncidentStatus(String incidentUid, String newStatus) async {
    try {
      final api = ApiService();
      final response = await api.updateIncident({
        'uid': incidentUid,
        'status': newStatus,
      });
