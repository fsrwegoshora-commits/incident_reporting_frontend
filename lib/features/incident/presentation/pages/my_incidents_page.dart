...
class _MyIncidentsScreenState extends State<MyIncidentsScreen> {
  ...
  Future<void> _loadUserIncidents() async {
    ...
    try {
      ...
      _currentUserUid = meResponse['data']?['uid'];

      // Load incidents
      final response = await api.getMyIncidents(page: 0, size: 20);

      if (response['status'] == 'Error') {
        _showSnackBar(response['message'] ?? 'Failed to load incidents', isError: true);
        return;
      }

      if (response['status'] == 'Success') {
        var rawContent = response['data'];
        ...
        print('Total elements: ${response['elements']}');
      }
    } catch (e) {
