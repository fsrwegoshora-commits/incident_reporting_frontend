...
import 'package:incident_reporting_frontend/features/incident/presentation/pages/report_incident_page.dart';
...
import 'package:incident_reporting_frontend/core/utils/auth_utils.dart';
import 'package:incident_reporting_frontend/core/theme/app_theme.dart';
import 'package:incident_reporting_frontend/features/profile/presentation/pages/admin_settings_page.dart';
import 'package:incident_reporting_frontend/features/agency/presentation/pages/agency_management_page.dart';
import 'package:incident_reporting_frontend/features/department/presentation/pages/department_management_page.dart';
import 'package:incident_reporting_frontend/features/incident/presentation/pages/my_incidents_page.dart';
import 'package:incident_reporting_frontend/features/incident/presentation/pages/officer_incidents_page.dart';
import 'package:incident_reporting_frontend/features/notification/presentation/pages/notifications_page.dart';
import 'package:incident_reporting_frontend/features/profile/presentation/pages/user_management_page.dart';
import 'package:incident_reporting_frontend/features/station/presentation/pages/police_station_management_page.dart';
import 'package:incident_reporting_frontend/features/home/presentation/widgets/improved_shift_card.dart';
import 'package:incident_reporting_frontend/features/emergency/presentation/pages/vehicle_management_page.dart';
import 'package:incident_reporting_frontend/features/emergency/presentation/pages/dispatch_center_page.dart';
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  int _stationOfficersCount = 0;
  int _onDutyOfficersCount = 0;
  int _totalUsersCount = 0;
  int _totalStationsCount = 0;
  ...
  Future<void> _loadIncidentStats() async {
    ...
    try {
      ...
      if (_userRole == "STATION_ADMIN" || _userRole == "ROOT" ||
          _isFireOrMedicalAdmin) {
        response = await api.getStationIncidents(page: 0, size: 100);
      } else if (_userRole == "POLICE_OFFICER" || _isFireOrMedicCrew) {
        response = await api.getOfficerIncidents(page: 0, size: 100);
      } else {
        response = await api.getMyIncidents(page: 0, size: 100);
      }
      ...
      final List incidents = (response['data'] is List) ? response['data'] : [];

      print('✅ Incidents fetched: ${incidents.length}');

      final int total = incidents.length;
      final int resolved = incidents.where((i) => i['status'] == 'RESOLVED').length;
      final int pending = incidents.where((i) => i['status'] == 'PENDING').length;
      final int inProgress = incidents.where((i) => i['status'] == 'IN_PROGRESS').length;
      ...
      print('Total: $total | Resolved: $resolved | Pending: $pending | In Progress: $inProgress');
...
// -----------------------------
  ...
  Future<void> _loadAdminStats() async {
    if (_userRole != "STATION_ADMIN" && _userRole != "ROOT" &&
        !_isFireOrMedicalAdmin) return;
    setState(() => _isLoadingAdminStats = true);
    try {
      final api = ApiService();

      if (_isFireOrMedicalAdmin && _stationUid != null) {
        // Vehicle counts for fire/medical station admin
        final vehiclesRes = await api.getVehiclesByStation(
            _stationUid!, page: 0, size: 1);
        if (vehiclesRes['status'] != 'Error') {
          setState(() =>
              _stationOfficersCount = vehiclesRes['elements'] ?? 0);
        }
        final vehiclesAllRes = await api.getAvailableVehicles();
        if (vehiclesAllRes['status'] != 'Error') {
          final all = List<Map<String, dynamic>>.from(
              vehiclesAllRes['data'] ?? []);
          setState(() => _onDutyOfficersCount = all.length);
        }
      } else if (_userRole == "STATION_ADMIN" && _stationUid != null) {
        // Total officers at this station
        final officersCountRes = await api.getPoliceOfficersByStation(
            _stationUid!, page: 0, size: 1);
        if (officersCountRes['status'] != 'Error') {
          setState(() =>
              _stationOfficersCount = officersCountRes['elements'] ?? 0);
        }

        // On-duty officers (fetch all, filter isOnDuty)
        final officersRes = await api.getPoliceOfficersByStation(
            _stationUid!, page: 0, size: 500);
        if (officersRes['status'] != 'Error') {
          final officers =
              List<Map<String, dynamic>>.from(officersRes['data'] ?? []);
          setState(() =>
              _onDutyOfficersCount =
                  officers.where((o) => o['isOnDuty'] == true).length);
        }
      } else if (_userRole == "ROOT") {
        // Total users across system
        final usersRes = await api.getUsers(size: 1);
        if (usersRes['status'] != 'Error') {
          setState(() => _totalUsersCount = usersRes['elements'] ?? 0);
        }

        // Total stations across system
        final stationsRes = await api.getPoliceStations(size: 1);
        if (stationsRes['status'] != 'Error') {
          setState(() => _totalStationsCount = stationsRes['elements'] ?? 0);
        }
      }
    } catch (e) {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (_userRole == "STATION_ADMIN" || _userRole == "ROOT" ||
          _isFireOrMedicalAdmin) {
        _loadAdminStats(); // fire and forget — updates UI when ready
      }

      if ((_userRole == "POLICE_OFFICER" || _isFireOrMedicCrew) &&
          _officerUid != null) {
  ...
  Future<void> _loadUserProfile() async {
    ...
    if (user != null) {
      ...
      if (_stationUid == null &&
        ...
        final stationResponse = await api.getStationsByAdmin();
        final stations = stationResponse['data'];
  ...
  Future<void> _fetchOfficerShifts() async {
    ...
    try {
      final api = ApiService();
      final response = await api.getShiftsByOfficer(_officerUid!, page: 0, size: 10);

      if (response['status'] == 'Error') return;

      final shifts = response['data'] as List<dynamic>?;
      if (shifts != null) {
  ...
  Future<void> _fetchNearbyPoliceStations(
    ...
    try {
      final api = ApiService();
      final response = await api.getNearbyPoliceStations(latitude, longitude, _maxDistance);
      ...
      final stations = response['data'] as List<dynamic>?;
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      final api = ApiService();
      final response = await api.deleteMyAccount();
      ...
      if (response['status'] == 'Error') {
        final errorMessage = response['message'] ?? 'Failed to delete account';
  ...
  String _getRoleDisplayName(String? role) {
    switch (role) {
      ...
      case "FIRE_OFFICER": return "Fire Officer";
      case "FIRE_STATION_ADMIN": return "Fire Admin";
      case "MEDIC": return "Medic";
      case "MEDICAL_STATION_ADMIN": return "Medical Admin";
      default: return "User";
    }
  }

  Color _getStatusColor() {
    if (_userRole == "POLICE_OFFICER" || _userRole == "FIRE_OFFICER" ||
        _userRole == "MEDIC") {
  ...
  bool get _isFireStationAdmin => _userRole == "FIRE_STATION_ADMIN";
  bool get _isMedicalStationAdmin => _userRole == "MEDICAL_STATION_ADMIN";
  bool get _isFireOrMedicalAdmin =>
      _isFireStationAdmin || _isMedicalStationAdmin;
  bool get _isFireOrMedicCrew =>
      _userRole == "FIRE_OFFICER" || _userRole == "MEDIC";
                  ...
                  children: [
                    ...
                    if (_userRole == "POLICE_OFFICER" || _isFireOrMedicCrew)
                      _buildShiftsSection()
                        ...
                        _userRole == "ROOT")
                      _buildAdminSection()
                    else if (_isFireOrMedicalAdmin)
                      _buildFireMedAdminSection()
              ...
              children: [
                ...
                Row(
                  children: [
                    Flexible(
                      child: Text(
                        _userName?.split(' ').first ?? _getRoleDisplayName(_userRole),
                        style: GoogleFonts.poppins(
                          fontSize: 17,
                          fontWeight: FontWeight.w700,
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text(
                    _isFireOrMedicCrew ? 'My Duty Schedule' : 'My Shifts',
                    style: GoogleFonts.poppins(
                      fontSize: 16,
                      fontWeight: FontWeight.w700,
                      color: Color(0xFF1A1F36),
                    ...
                    children: [
                      Expanded(
                        child: _buildStatCard(
                          ...
                          value: _isLoadingAdminStats ? '…' : _getTotalUsersCount(),
                          label: 'Total Users',
                          color: Colors.white.withOpacity(0.9),
                        ),
                      ),
                      ...
                      Expanded(
                        child: _buildStatCard(
                          ...
                          value: _isLoadingAdminStats ? '…' : _getTotalStationsCount(),
                          label: 'All Stations',
                          color: Colors.white.withOpacity(0.9),
                        ),
                      ),
                      ...
                      Expanded(
                        child: _buildStatCard(
                          ...
                          value: _isLoadingIncidentStats
                              ? '…'
                              : '${_incidentStats?['total'] ?? 0}',
                          label: 'Total Reports',
                          color: Colors.white.withOpacity(0.9),
                        ),
                      ),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: _buildStatCard(
                          icon: Icons.people_outline_rounded,
                          value: _isLoadingAdminStats ? '…' : _getStationUsersCount(),
                      ...
                      Expanded(
                        child: _buildStatCard(
                          ...
                          value: _isLoadingAdminStats ? '…' : _getOnDutyCount(),
                          label: 'On Duty',
                          color: Colors.white.withOpacity(0.9),
                        ),
                      ),
...
// ============================================================================
// FIRE / MEDICAL STATION ADMIN SECTION
// ============================================================================

  Widget _buildFireMedAdminSection() {
    final isFire = _isFireStationAdmin;
    final themeColor = isFire ? const Color(0xFFEF4444) : const Color(0xFF10B981);
    final gradientColors = isFire
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            isFire ? 'Fire Station Dashboard' : 'Medical Station Dashboard',
                            style: GoogleFonts.poppins(
                              fontSize: 20,
                              fontWeight: FontWeight.w700,
                              color: Colors.white,
                  ...
                  children: [
                    Expanded(
                      child: _buildStatCard(
                        ...
                        value: _isLoadingAdminStats ? '…' : '$_stationOfficersCount',
                        label: isFire ? 'Vehicles' : 'Ambulances',
                        color: Colors.white.withOpacity(0.9),
                      ),
                    ),
                    ...
                    Expanded(
                      child: _buildStatCard(
                        ...
                        value: _isLoadingAdminStats ? '…' : '$_onDutyOfficersCount',
                        label: 'Available',
                        color: Colors.white.withOpacity(0.9),
                      ),
                    ),
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      isFire ? 'Fire Station Management' : 'Medical Station Management',
                      style: GoogleFonts.poppins(
                        fontSize: 16,
                        fontWeight: FontWeight.w700,
                  ...
                  children: [
                    _buildModernAdminCard(
                      icon: isFire
                          ? Icons.fire_truck_rounded
                          : Icons.medical_services_rounded,
                      title: isFire ? 'Vehicles' : 'Ambulances',
                    ...
                    _buildModernAdminCard(
                      ...
                      onTap: () => Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) =>
                              DispatchCenterPage(stationUid: _stationUid),
                        ),
                      ),
                    ),
                    _buildModernAdminCard(
...
// ============================================================================
  ...
  String _getTotalUsersCount() => '$_totalUsersCount';

  String _getTotalStationsCount() => '$_totalStationsCount';

  String _getStationUsersCount() => '$_stationOfficersCount';

  String _getStationIncidentsCount() {
    final pending = _incidentStats?['pending'] ?? 0;
    final inProgress = _incidentStats?['in_progress'] ?? 0;
    return '${pending + inProgress}';
  }

  String _getOnDutyCount() => '$_onDutyOfficersCount';
