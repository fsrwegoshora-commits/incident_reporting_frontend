...
import 'package:incident_reporting_frontend/features/department/presentation/pages/department_form.dart';
...
class _DepartmentManagementScreenState extends State<DepartmentManagementScreen>
  ...
  Future<Map<String, dynamic>> _fetchDepartments() async {
    ...
    try {
      ...
      final response = await _api.getDepartments(page: _currentPage, size: _pageSize);
  ...
  void _deleteDepartment(String uid) async {
    ...
    if (confirm == true) {
      try {
        final response = await _api.deleteDepartment(uid);
