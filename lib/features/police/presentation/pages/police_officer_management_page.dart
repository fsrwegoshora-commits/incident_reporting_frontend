...
import 'package:incident_reporting_frontend/features/police/presentation/pages/register_police_officer_tab.dart';
...
class _PoliceOfficerManagementScreenState extends State<PoliceOfficerManagementScreen>
  ...
  Future<void> _fetchPoliceOfficers() async {
    ...
    try {
      final response = await _api.getPoliceOfficers(
        page: _currentPage,
        size: _pageSize,
      );
      ...
      final policeOfficers = response['data'] ?? [];
  ...
  Future<void> _deletePoliceOfficer(String uid) async {
    ...
    if (confirm == true) {
      try {
        final response = await _api.deletePoliceOfficer(uid);
        ...
        final isSuccess = response['status'] == 'Success' || message.toLowerCase().contains('success');
